...
// can hold each one to its habit. M1 ships the Moron; the others arrive in M3.
import { WEAPON_ORDER, alive, trajectory, COLS, TANK_W } from './scorched.js';

/** The Moron: a random angle and power, the cheapest shell. The original's easiest opponent. */
export function moron(g, tank, rnd = g.rnd) {
  ...
  return { angle, power, weapon: 'babyMissile' };
}

/**
 * The Shooter's aim (used from M3, exported now so its test can grow with it): aims at the
 * nearest living tank by searching power at a fixed angle for the landing nearest the target,
 * with this game's gravity and wind, then corrects from the last miss. Here in M1 it is only the
 * search, so the Moron has something to be compared against.
 */
export function solve(g, tank, target, angles = [35, 45, 55, 65, 75]) {
  ...
  for (const a0 of angles) {
    ...
    for (let power = 150; power <= 1000; power += 25) {
      ...
      // where the shot comes down to the target's height, or the ground
...
export function decide(g, tank) {
  ...
  const d = play(g, tank);
  if (!WEAPON_ORDER.includes(d.weapon) || (tank.inventory[d.weapon] ?? 0) <= 0) d.weapon = 'babyMissile';
  return d;
}
