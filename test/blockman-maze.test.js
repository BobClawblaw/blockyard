...
test('the pen sits in the middle with one gate, the start below it, and the tunnels at its height', () => {
  assert.equal(m.pen.length, 16, 'an eight-wide, two-deep pen');
...
test('four pellets, out in the quarters, each a short run from a junction', () => {
  ...
  for (const p of m.pellets) {
    ...
    assert.ok(junctions(p) === 2, 'on a corridor, not at a junction: taking it is a commitment');
  }
...
test('the dot budget lands where the tables expect, and the walls are the rest', () => {
  ...
  assert.ok(bare >= 8 && bare <= 40, `${bare} bare tiles: the pen surround, the tunnel run and the start`);
  for (const t of m.pen) assert.equal(m.at(t.x, t.y), WALL, 'the pen itself is solid to BlockMan');
});

test('two no-upward-turn tiles above the pen and two below, mirrored', () => {
  const up = noUpTurn(m);
  assert.equal(up.length, 4);
  for (const t of up) assert.equal(m.at(t.x, t.y), OPEN, 'they are corridor tiles');
  const above = up.filter((t) => t.y < m.door.y), below = up.filter((t) => t.y > m.door.y);
  assert.equal(above.length, 2);
  assert.equal(below.length, 2);
  for (const pair of [above, below]) assert.equal(pair[0].x + pair[1].x, m.w - 1, 'a mirrored pair');
});

test('the outer loop can be run forever, and it cannot be run safely', () => {
  // a loop: walk the outermost corridor and come back to the start
  const ring = [];
  for (let x = 0; x < m.w; x++) if (m.at(x, 1) === OPEN) ring.push({ x, y: 1 });
  assert.equal(ring.length, m.w - 2, 'the top corridor runs the full width');
  for (let x = 0; x < m.w; x++) if (m.at(x, m.h - 2) === OPEN) assert.ok(true);
  const bottom = [...Array(m.w).keys()].filter((x) => m.at(x, m.h - 2) === OPEN);
  assert.equal(bottom.length, m.w - 2, 'so does the bottom');
  // dead ends: tiles with exactly one open neighbour. Some, or the loop is all there is.
  let deadEnds = 0;
  for (let y = 0; y < m.h; y++) {
    for (let x = 0; x < m.w; x++) {
      if (m.at(x, y) === WALL) continue;
      let n = 0;
      for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) if (m.at(x + dx, y + dy) === OPEN) n += 1;
      if (n === 1) deadEnds += 1;
    }
  }
  assert.ok(deadEnds >= 4, `${deadEnds} dead ends: a maze with none is a racetrack`);
});
