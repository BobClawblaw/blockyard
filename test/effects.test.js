...
test('there are at least twenty-five effects, and every one has a switch of its own', () => {
  ...
  assert.ok(MARKET_FX.every((k) => SPACE_FX.includes(k) || ['pulse', 'bulge', 'breathe', 'saber'].includes(k)), 'the price board offers nothing the block board lacks but the four of its own');
  ...
  assert.deepEqual(FX_KINDS.filter((k) => !SPACE_FX.includes(k)), ['pulse', 'bulge', 'breathe', 'saber'], 'the block board lacks only the four that are the price board\'s own (the black hole hovers over it too since 2026-09-15)');
...
test('every effect lights something at some point in its run, and nothing before or after it', () => {
  ...
  for (const kind of FX_KINDS) {
    if (kind === 'pulse' || kind === 'bulge' || kind === 'breathe' || kind === 'saber') continue;   // the price board's own
    let lit = 0, worst = 0;
