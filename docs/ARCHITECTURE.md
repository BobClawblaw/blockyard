...
- **Pool labels** are read from `pool-map.json` (generated by
  `scripts/pool-map.js`) and `pool-aliases.json` (edited by hand). Both are
  optional. When neither exists, the coinbase tag is shown exactly as the miner
  wrote it.
...
| `slow` | 60 s | `getindexinfo`, `gettxoutsetinfo`, `getchaintxstats 120` | 5 | UTXO set, indexes, tx rate |
...
- **`collect/sync.js`**: the sync state machine (`unknown`, `ibd`, `catching_up`,
  ...
  without a measured rate over at least 60 s of samples. It is pure and
  unit-tested.
