...
| **Detailed** | Every transaction in the next block on a 96-unit grid: larger ones as their own squares, runs of small ones at the same feerate bundled into larger squares (hover one to see how many), drawn as low slabs so that thousands of them stay readable. The best view for the block's texture. |
