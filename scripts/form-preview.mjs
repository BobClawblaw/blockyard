// THE FORMATION, FRAME BY FRAME, IN A REAL BROWSER. Serves public/, starts a headless chromium, and
// draws formgl.js alone (no board over it) at chosen moments of its loop, writing each as a PNG --
// because a sky is judged by looking at it, next to the film it is after.
//
//   node scripts/form-preview.mjs [--out <dir>] [--at 0.1,0.3,0.5,0.7,0.86] [--size 1280x720] [--gpu vulkan]
...
const OUT = arg('--out', path.join(os.tmpdir(), 'blockyard-form-preview'));
const AT = arg('--at', '0.1,0.3,0.5,0.7,0.86').split(',').map(Number);
...
const TIMES = arg('--times', '') ? arg('--times', '').split(',').map(Number) : null;
const SPEED = Number(arg('--speed', '1'));
const PLACE = arg('--place', 'center');
const BRIGHT = Number(arg('--brightness', '0.5')), FLOW = Number(arg('--flow', '1'));
const FALLBACK = process.argv.includes('--fallback');     // draw galform.js, the 2D sky for machines with no graphics card                   // center | top-left | top-right | bottom-left | bottom-right
...
try {
  const { formGlAttach, FORM_CONST } = await import('/js/formgl.js');
  const canvas = document.getElementById('c');
  if (${FALLBACK}) {
    const { drawGalaxyForm } = await import('/js/galform.js');
    canvas.width = ${SW}; canvas.height = ${SH};
    const c2 = canvas.getContext('2d');
    for (const u of ${JSON.stringify(TIMES ?? AT)}) {
      const t0 = performance.now();
      c2.fillStyle = '#000'; c2.fillRect(0, 0, ${SW}, ${SH});
      drawGalaxyForm(c2, ${SW}, ${SH}, 1, ${TIMES ? 'u' : 'u * FORM_CONST.FORM_CYCLE * 1000'}, { formSpeed: ${SPEED}, formBrightness: ${BRIGHT}, formFlow: ${FLOW}, formAt: ${JSON.stringify(PLACE)} });
  ...
  const ctl = formGlAttach(canvas, { preserve: true, allowSoftware: true, onCompileError: (l) => out.errors.push('compile: ' + l) });
  if (!ctl) out.errors.push('no controller');
  else for (const u of ${JSON.stringify(TIMES ?? AT)}) {
    const t0 = performance.now();
    ctl.draw(${SW}, ${SH}, 1, ${TIMES ? 'u' : 'u * FORM_CONST.FORM_CYCLE * 1000'}, { formSpeed: ${SPEED}, formBrightness: ${BRIGHT}, formFlow: ${FLOW}, formAt: ${JSON.stringify(PLACE)} });
  ...
  if (ctl) {
    ...
    const t0 = performance.now();
    for (let i = 0; i < 60; i++) ctl.draw(${SW}, ${SH}, 1, 60000 + i * 40, { formSpeed: 1 });
    gl.readPixels(0, 0, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, px);
...
const profile = path.join(existsSync(snapHome) ? snapHome : os.tmpdir(), `blockyard-form-preview-${process.pid}`);
...
try {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const file = path.join(OUT, `form-${String(s.u).replace('.', '_')}.png`);
