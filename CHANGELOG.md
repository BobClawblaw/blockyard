...
- Viewer modes: **Simple** (the richest few hundred transactions as cubes, the rest as
  ...
  **Detailed** (every transaction in the next block on a 24-unit grid, small ones bundled,
  in feerate order, one bundle per grid unit, drawn as low slabs, area-true so a full block
  fills the board),
