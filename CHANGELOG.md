...
- **Added: BlockMan**, a maze chase on the block engine, under Diversions after Scorched Yard. The genre the 1980 arcade game invented, built from nothing here: one maze of 28 by 28 tiles (264 dots, four pellets, a pen, a tunnel out each side) with no dead end in it, no lane running the full width of the board, and no band of wall crossed in fewer than four places — drawn on a lattice and then closed up passage by passage, each closure kept only while no dead end, open room or cut-off corner appears — kept for every level as the original kept its own, built once as 498 cubes with everything that moves painted over them in the same light — 60 frames a second in the browser, median frame 16.7 ms, with five actors and 264 dots on screen. The four pursuers run one rule each — Chaser at your tile, Ambusher four tiles ahead of you, Flanker through the far side, Wanderer in and out at eight tiles — over eight scatter/chase waves that reverse them where they stand, with pen dot counters, tunnel slowdown, Chaser's two speed steps, a pellet whose fright shortens each level and stops mattering at nineteen, the 200-1600 ladder, a walk home as eyes on a distance field, fruit twice a level, an extra life at 10,000 and the top eight games kept. Cornering and queued turns, cheat mode (**T**) ringing each pursuer's target, three difficulties, and an attract mode that plays itself for a wall screen. Its sound is a **three-voice wavetable generator** of our own (`public/js/blockmansound.js`): the technique the era's hardware used — three channels stepping a short table at a rate set by a frequency register — with every table computed from a formula in the file and every sound our own choice of pitches, including a pulse whose tempo carries how many dots are left. Nothing in it came from that game: not the maze, not the characters, not a note, not a byte of ROM, so it ships Apache 2.0 like the rest (`docs/PLAN-BLOCKMAN.md` draws the line, and `docs/USER-GUIDE.md` has the section)

- **Scoped: BlockMan**, a maze chase on the block engine (`docs/PLAN-BLOCKMAN.md`). The plan draws the line between the genre's mechanics, which anyone may implement, and one company's expression, which nobody may copy: one hand-drawn maze of our own, kept for every level as the original did, cubes for characters, our own sounds and names, no ROM anywhere near it, so the whole thing stays Apache 2.0. It also measures what the engine can draw: a block per arcade pixel is 64,512 blocks at 1.4 frames a second, so the maze is a block layer redrawn once a level and everything that moves is painted over it through the renderer's overlay hook, the way Scorched Yard paints its shells
